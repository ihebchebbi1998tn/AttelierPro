    ...
    switch($method) {
        case 'GET':
            if (isset($_GET['id'])) {
                ...
                $clientEmail = isset($_GET['client_email']) ? $_GET['client_email'] : null;
                ...
                if ($clientEmail) {
                    // Filter by client email (for soustraitance users)
                    $whereClause = "WHERE sc.email = ?";
                    $params = [$clientEmail];
                } elseif ($clientId) {
                    // Filter by client ID (for admin users)
                    $whereClause = "WHERE sp.client_id = ?";
                    $params = [$clientId];
                }
                ...
                // Get total count
                $countQuery = "SELECT COUNT(*) as total FROM production_soustraitance_products sp 
                              LEFT JOIN production_soustraitance_clients sc ON sp.client_id = sc.id 
                              $whereClause";
        ...
        case 'POST':
            ...
            // Check if we have form data (multipart) or JSON
            $input = [];
            
            // If POST data exists, use it (multipart/form-data)
            if (!empty($_POST)) {
                $input = $_POST;
            } else {
            ...
            if (!isset($input['client_id']) || !isset($input['nom_product'])) {
                error_log('Missing required fields. Input: ' . print_r($input, true));
