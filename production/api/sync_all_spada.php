...
try {
    ...
    // Fetch all products from Spadadibattaglia API
    $api_url = 'https://spadadibattaglia.com/api/get_all_products_admin.php';
    ...
    if ($response === false) {
        throw new Exception('Failed to fetch data from Spadadibattaglia API');
    }
    
    $data = json_decode($response, true);
    
    if (!$data || !isset($data['data']) || !is_array($data['data'])) {
        throw new Exception('Invalid response format from Spadadibattaglia API');
    }
    ...
    foreach ($products as $product) {
        ...
        $check_stmt = $conn->prepare("SELECT id FROM productions_products_spada WHERE external_product_id = ?");
        ...
        if ($existing) {
            ...
            $update_sql = "UPDATE productions_products_spada SET 
            ...
            // Insert new product
            $insert_sql = "INSERT INTO productions_products_spada (
            ...
            $insert_sql .= ", boutique_origin) VALUES (?" . str_repeat(', ?', 21 + count($size_fields)) . ", 'spadadibattaglia')";
