...
switch($method) {
    case 'GET':
        if(isset($_GET['product_id'])) {
            // Get measurements for specific soustraitance product
            $stmt = $db->prepare("SELECT * FROM production_soustraitance_products_mesure WHERE product_id = ? ORDER BY measurement_name ASC");
            $stmt->execute([$_GET['product_id']]);
            $measurements = $stmt->fetchAll();
            echo json_encode(['success' => true, 'data' => $measurements]);
        } else {
            // Get all measurements with product and client info
            $stmt = $db->query("
                SELECT m.*, p.nom_product, p.reference_product, c.name as client_name
                FROM production_soustraitance_products_mesure m
                JOIN production_soustraitance_products p ON m.product_id = p.id
                LEFT JOIN production_soustraitance_clients c ON p.client_id = c.id
                ORDER BY c.name ASC, p.nom_product ASC, m.measurement_name ASC
            ");
            $measurements = $stmt->fetchAll();
            echo json_encode(['success' => true, 'data' => $measurements]);
        }
    ...
    case 'POST':
        ...
        $stmt = $db->prepare("INSERT INTO production_soustraitance_products_mesure (product_id, measurement_name, measurement_value, tolerance, unit, notes) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    case 'PUT':
        ...
        $stmt = $db->prepare("UPDATE production_soustraitance_products_mesure SET measurement_name=?, measurement_value=?, tolerance=?, unit=?, notes=? WHERE id=?");
    ...
    case 'DELETE':
        ...
        $stmt = $db->prepare("DELETE FROM production_soustraitance_products_mesure WHERE id = ?");
