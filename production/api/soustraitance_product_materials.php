    ...
    switch($method) {
        case 'GET':
            if (isset($_GET['product_id'])) {
                ...
                // Récupérer tous les liens produit-matériaux soustraitance
                $stmt = $db->prepare("
                    SELECT 
                        pm.*,
                        p.nom_product,
                        ...
                        qt.nom as quantity_type_name
                    FROM production_soustraitance_product_materials pm
                    JOIN production_soustraitance_products p ON pm.product_id = p.id
                    JOIN production_matieres m ON pm.material_id = m.id
                    JOIN production_quantity_types qt ON pm.quantity_type_id = qt.id
        ...
        case 'POST':
            ...
            if (isset($data['action']) && $data['action'] === 'configure_product_materials') {
                ...
                try {
                    ...
                    $deleteStmt = $db->prepare("DELETE FROM production_soustraitance_product_materials WHERE product_id = :product_id");
                    ...
                    foreach ($data['materials'] as $material) {
                        $insertStmt = $db->prepare("
                            INSERT INTO production_soustraitance_product_materials 
                    ...
                    // Marquer le produit soustraitance comme configuré
                    $updateStmt = $db->prepare("UPDATE production_soustraitance_products SET materials_configured = 1 WHERE id = :product_id");
                    ...
                    $verifyStmt = $db->prepare("SELECT materials_configured FROM production_soustraitance_products WHERE id = :product_id");
                    $verifyStmt->bindParam(':product_id', $product_id, PDO::PARAM_INT);
                    $verifyStmt->execute();
                    $result = $verifyStmt->fetch();
                    error_log("Updated materials_configured to: " . $result['materials_configured'] . " for soustraitance product ID: " . $product_id);
                    
                    $db->commit();
                    echo json_encode(['success' => true, 'message' => 'Configuration des matériaux mise à jour avec succès']);
                } catch (Exception $e) {
                    $db->rollback();
                    error_log("Soustraitance materials error: " . $e->getMessage());
        ...
        case 'PUT':
            ...
            $stmt = $db->prepare("
                UPDATE production_soustraitance_product_materials SET 
        ...
        case 'DELETE':
            ...
            $stmt = $db->prepare("DELETE FROM production_soustraitance_product_materials WHERE id = :id");
