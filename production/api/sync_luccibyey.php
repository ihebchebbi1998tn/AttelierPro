...
try {
    // Récupérer les produits depuis l'API Lucci By Ey
    $apiUrl = 'https://luccibyey.com.tn/api/get_auto_replenishment_products.php';
    $response = file_get_contents($apiUrl);
    
    if ($response === false) {
        throw new Exception('Impossible de récupérer les données depuis l\'API Lucci By Ey');
    }
    
    $data = json_decode($response, true);
    
    if (!$data || !$data['success']) {
        throw new Exception('Réponse invalide de l\'API Lucci By Ey');
    }
    ...
    foreach ($products as $product) {
        ...
        $checkStmt = $db->prepare("SELECT id FROM production_ready_products WHERE boutique_origin = 'luccibyey' AND external_product_id = :external_id");
    ...
    $logStmt = $db->prepare("
        ...
        VALUES ('luccibyey', 'manual', :products_found, :products_added, :products_updated, 'success', :duration, :user_id)
    ");
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Synchronisation Lucci By Ey terminée avec succès',
        'data' => [
            'boutique' => 'luccibyey',
    ...
    $logStmt = $db->prepare("
        ...
        VALUES ('luccibyey', 'manual', 'error', :error_message, :duration, :user_id)
    ");
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Erreur lors de la synchronisation Lucci By Ey: ' . $e->getMessage()
    ]);
}
?>