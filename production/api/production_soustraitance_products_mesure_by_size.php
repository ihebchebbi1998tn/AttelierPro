...
switch($method) {
        ...
        if(isset($_GET['product_id'])) {
            ...
            $stmt = $db->prepare("
                SELECT 
                    ...
                    created_at,
                    updated_at
                FROM production_soustraitance_products_mesure 
                WHERE product_id = ? 
        ...
        try {
            ...
            foreach ($measurements as $measurementData) {
                ...
                foreach ($sizes as $sizeValue => $measurementValue) {
                    ...
                    $stmt = $db->prepare("
                        INSERT INTO production_soustraitance_products_mesure 
    ...
    case 'DELETE':
        ...
        $stmt = $db->prepare("DELETE FROM production_soustraitance_products_mesure WHERE product_id = ? AND measurement_name = ?");
