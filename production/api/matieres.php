...
switch($method) {
    case 'GET':
        if(isset($_GET['id'])) {
            ...
            // Get replaceable materials
            $stmt = $db->query("SELECT * FROM production_matieres WHERE active = 1 ORDER BY nom");
            $materials = $stmt->fetchAll();
            echo json_encode(['success' => true, 'data' => $materials]);
            
        } elseif(isset($_GET['low_stock'])) {
            // Get materials with low stock (critical + warning)
            $stmt = $db->query("SELECT * FROM production_matieres WHERE quantite_stock <= quantite_min ORDER BY quantite_stock ASC");
            $materials = $stmt->fetchAll();
