...
switch($method) {
    case 'GET':
        if(isset($_GET['product_id'])) {
            ...
            $stmt = $db->prepare("
                SELECT id, product_id, filename, original_filename, file_path, file_type, 
    ...
    case 'POST':
        ...
        $file = $_FILES['file'];
        $upload_dir = 'uploads/product_attachments/';
        ...
        if (move_uploaded_file($file['tmp_name'], $file_path)) {
            try {
                ...
                $db->exec("CREATE TABLE IF NOT EXISTS production_products_attachments (
                ...
                // Save to database
                $stmt = $db->prepare("
                    INSERT INTO production_products_attachments 
                    (product_id, filename, original_filename, file_path, file_type, file_size, mime_type, description, uploaded_by, created_date, modified_date) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
                ");
    ...
    case 'DELETE':
        ...
        $stmt = $db->prepare("SELECT file_path FROM production_products_attachments WHERE id = ?");
        $stmt->execute([$input['id']]);
        $attachment = $stmt->fetch();
        
        if ($attachment) {
            // Delete from database
            $stmt = $db->prepare("DELETE FROM production_products_attachments WHERE id = ?");
