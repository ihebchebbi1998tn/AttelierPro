...
const ConfigurerMateriauxSoustraitance = () => {
  ...
  const openMaterialConfig = (material: Material) => {
    ...
    if (existingMaterials.length > 0) {
      // Load existing configuration - just take the first one's quantity
      setTempConfig({
        quantity_type_id: existingMaterials[0].quantity_type_id,
        quantity: existingMaterials[0].quantity_needed,
        commentaire: existingMaterials[0]?.commentaire || ''
      });
    } else {
      // Initialize with default quantity (new material)
      setTempConfig({
  ...
  const saveMaterialConfig = async () => {
    ...
    // Create a single ProductMaterial entry (no size-specific)
    const newMaterials: ProductMaterial[] = [{
      material_id: selectedMaterial.id,
      quantity_needed: tempConfig.quantity,
      ...
      <Dialog open={showConfigModal} onOpenChange={setShowConfigModal}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Configurez la quantité nécessaire de ce matériau
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <Input
                  ...
                  min="0"
                  step="0.001"
                  value={tempConfig.quantity}
                  onChange={(e) => setTempConfig({...tempConfig, quantity: parseFloat(e.target.value) || 0})}
