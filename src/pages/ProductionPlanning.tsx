      ...
      <div className="bg-card/95 backdrop-blur-md border-b shadow-sm sticky top-0 z-50">
        <div className="container-responsive py-2 lg:py-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
            <Button variant="outline" size="sm" onClick={() => navigate(`/produits/${id}`)} className="hover:bg-accent shadow-sm shrink-0">
                ...
                <div className="lg:col-span-3 space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Badge className={`text-sm font-medium px-3 py-1 ${product.boutique_origin === 'luccibyey' ? 'bg-primary/10 text-primary border-primary/20' : 'bg-secondary/50 text-secondary-foreground border-secondary'}`}>
                        {product.boutique_origin === 'luccibyey' ? 'Lucci By Ey' : 'Spada di Battaglia'}
                      </Badge>
                    </div>
                  </div>
                  
                  {(product.price_product || product.category_product) && <Separator className="my-4" />}
                  
                  {(product.price_product || product.category_product) && <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {product.price_product && <div className="space-y-2">
                          <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                            Prix Unitaire
                          </Label>
                      ...
                      {product.category_product && <div className="space-y-2">
                          <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                            Catégorie
                          </Label>
            ...
            <CardContent className="p-6">
              ...
              {totalPlanned > 0 && <div className="mt-8 bg-gradient-to-r from-primary/10 via-primary/5 to-primary/10 border border-primary/20 rounded-xl p-6">
                  <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                    ...
                    <Button onClick={validateProduction} disabled={validating} size="lg" className="flex items-center gap-3 bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg hover:shadow-xl transition-all duration-200 px-8 py-3">
                      {validating ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" /> : <CheckCircle className="h-5 w-5" />}
        ...
        <DialogContent className="sm:max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="pb-6">
            <DialogTitle className="flex items-center gap-3 text-xl">
              {validationResult?.has_sufficient_stock ? <div className="p-2 bg-success/20 rounded-lg">
                  <CheckCircle className="h-6 w-6 text-success" />
                </div> : <div className="p-2 bg-warning/20 rounded-lg">
              ...
              {validationResult && <div className="space-y-8">
                   {/* Simplified Summary */}
                   <div className="p-6 rounded-lg border bg-card">
                     <div className="flex items-start gap-4">
                       <div className="p-2 rounded-lg bg-muted">
                         {validationResult.has_sufficient_stock ? <CheckCircle className="h-5 w-5 text-foreground" /> : <AlertTriangle className="h-5 w-5 text-foreground" />}
                       </div>
                       <div className="flex-1">
                         <h3 className="text-lg font-semibold mb-2 text-foreground">
                           {validationResult.has_sufficient_stock ? 'Stock Suffisant - Production Autorisée' : 'Stock Insuffisant - Ajustement Nécessaire'}
                         </h3>
                         <p className="text-sm leading-relaxed text-muted-foreground">
                           {validationResult.has_sufficient_stock ? 'Tous les matériaux sont disponibles en quantité suffisante. Vous pouvez procéder à la production.' : 'Certains matériaux ne sont pas disponibles en quantité suffisante. Consultez les détails ci-dessous.'}
                         </p>
                       </div>
                     </div>
                   </div>
                   ...
                   {validationResult.validation_warnings && validationResult.validation_warnings.length > 0 && <div className="border rounded-lg p-6 bg-card">
                       ...
                       <div className="space-y-3">
                         {validationResult.validation_warnings.map((warning, index) => <div key={index} className="border rounded-lg p-4 bg-muted/30">
                             <p className="text-sm text-foreground font-medium">{warning.message}</p>
                             {warning.type === 'impossible_production' ? <div className="mt-3 p-3 bg-card rounded-md">
              ...
              <div>
                ...
                <div className="grid gap-6">
                  {Object.entries(validationResult.material_requirements).map(([materialId, requirement]) => <Card key={materialId} className="modern-card">
                      <CardContent className="p-6">
                ...
                {requirement.is_sufficient ? '0' : formatNumber(requirement.total_needed - requirement.current_stock)}
                         ...
                         <div className="mb-4">
                           ...
                           <div className="relative h-3 w-full overflow-hidden rounded-full bg-secondary">
                             <div className={`h-full transition-all ${requirement.stock_status === 'critical' ? 'bg-destructive' : requirement.stock_status === 'warning' ? 'bg-warning' : 'bg-success'}`} style={{
                        width: `${Math.min(requirement.stock_percentage, 100)}%`
                      }} />
                           </div>
                        ...
                        <div>
                          <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-3">
                           ...
                           <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                             {requirement.size_breakdown.map((size, index) => <div key={index} className="bg-muted/20 border rounded-lg p-3 text-center">
              ...
              {!validationResult.has_sufficient_stock && validationResult.can_produce_any && Object.keys(validationResult.suggested_quantities).length > 0 && <div>
                  <h3 className="text-lg font-semibold text-foreground mb-6 flex items-center gap-2">
                  ...
                  <Card className="modern-card border-warning/20">
                    <CardContent className="p-6">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
                        {Object.entries(validationResult.suggested_quantities).map(([size, suggestedQty]) => <div key={size} className="bg-card border border-border rounded-lg p-4 text-center">
                             <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-2">
                      ...
                      <Button onClick={applySuggestedQuantities} variant="outline" size="lg" className="w-full border-warning/30 text-warning hover:bg-warning/10 flex items-center gap-2" disabled={Object.values(validationResult.suggested_quantities).every(qty => qty === 0)}>
                        <CheckCircle className="h-4 w-4" />
                        {Object.values(validationResult.suggested_quantities).every(qty => qty === 0) ? 'Aucune Production Possible' : 'Appliquer les Quantités Optimisées'}
                      </Button>
                    </CardContent>
                  </Card>
                </div>}
          ...
          <DialogFooter className="gap-3 pt-6 border-t">
            <Button variant="outline" onClick={() => setShowValidationModal(false)} size="lg" className="flex-1 sm:flex-none">
              Modifier les Quantités
            </Button>
            {validationResult?.has_sufficient_stock && <Button onClick={() => {
            setShowValidationModal(false);
            setShowConfirmationModal(true);
          }} size="lg" className="flex-1 sm:flex-none bg-success hover:bg-success/90 text-success-foreground flex items-center gap-2 shadow-lg">
