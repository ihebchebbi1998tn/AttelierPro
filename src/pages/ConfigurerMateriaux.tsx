...
const ConfigurerMateriaux = () => {
  ...
  const handleValidateAndProduction = async () => {
    if (selectedMaterials.length === 0) {
      toast({
        title: "Aucun matériau configuré",
        description: "Veuillez configurer au moins un matériau",
        variant: "destructive"
      });
      return;
    }

    // Trigger one final save to ensure everything is persisted
    await autoSave(selectedMaterials);

    // Deduct stock when confirming production
    try {
      ...
      if (!data.success) {
        if (data.insufficient_materials) {
          ...
          toast({
            ...
            description: `Impossible de déduire le stock: ${insufficientList}`,
            variant: "destructive"
          });
        } else {
          toast({
            title: "Erreur",
            description: data.error || "Erreur lors de la déduction du stock",
            variant: "destructive"
          });
        }
        return;
      ...
      toast({
        title: "Confirmation de Production",
        description: `Stock déduit avec succès pour ${data.total_quantity_produced} pièce(s). ${data.transactions.length} matériau(x) traité(s).`
      });

      // Navigate back to product details
      navigate(`/produits/${productId}`, { state: { refresh: Date.now() } });

    } catch (error) {
      console.error('Error deducting stock:', error);
      toast({
        title: "Erreur",
        description: "Erreur lors de la déduction du stock",
        variant: "destructive"
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        {/* Valider Button - Confirmation de Production */}
        {distinctSelectedCount > 0 && (
          <Button
            onClick={handleValidateAndProduction}
            className="flex items-center gap-2"
            size="lg"
          >
