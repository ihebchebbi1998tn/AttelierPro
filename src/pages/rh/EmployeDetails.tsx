...
import { CustomWorkCalendar } from "@/components/CustomWorkCalendar";
import { format, differenceInCalendarDays, startOfDay, parseISO, isSameDay, isWeekend, isValid, differenceInMinutes } from "date-fns";
...
const EmployeDetails = () => {
  ...
  const [allPointageData, setAllPointageData] = useState<any[]>([]);
  ...
  useEffect(() => {
    // Filter pointage data by selected month
    if (allPointageData.length > 0) {
      const filtered = allPointageData.filter(p => p.month === selectedMonth);
      setPointageData(filtered);
    }
  }, [selectedMonth, allPointageData]);
  ...
  const loadPointageData = async () => {
    try {
      ...
      const response = await fetch(
        `https://luccibyey.com.tn/production/api/rh_employe_pointage.php?employee_id=${id}&month=${monthKey}`
      );
      const data = await response.json();
      if (data.success && data.data) {
        const filtered = data.data.filter((p: any) => p.month === monthKey);
        setPointageData(filtered);
      } else {
        setPointageData([]);
      }
  ...
  const loadAllPointageData = async () => {
    try {
      const response = await fetch(
        `https://luccibyey.com.tn/production/api/rh_employe_pointage.php?employee_id=${id}`
      );
      const data = await response.json();
      if (data.success && data.data) {
        setAllPointageData(data.data);
        // Extract unique months
        const months = [...new Set(data.data.map((p: any) => p.month))].sort().reverse();
        setAvailableMonths(months as string[]);
        // Set current month data
        const currentMonth = format(new Date(), 'yyyy-MM');
        const filtered = data.data.filter((p: any) => p.month === currentMonth);
        setPointageData(filtered);
      }
    } catch (error) {
      console.error("Error loading all pointage data:", error);
    }
  };
  ...
  const isDateWorkDay = (date: Date) => {
    // Check pointage data first
    const pointageEntry = pointageData.find(p => {
      if (!p.date) return false;
      const pointageDate = parseISO(p.date);
      return isSameDay(pointageDate, date);
    });

    if (pointageEntry) {
      // If jr_travaille > 0, it's a work day
      return parseFloat(pointageEntry.jr_travaille || 0) > 0;
    }

    // Fallback to original logic if no pointage data
    // Don't show as work day if it's a holiday
    if (isDateHoliday(date)) return false;
    
    // Check if there's a specific schedule for this date FIRST (before weekend check)
    // This allows weekend work to be marked as work days
    ...
    // Don't show weekends as work days (unless they have a schedule, checked above)
    if (isWeekend(date)) return false;
    
    // Don't show future dates as work days
    const today = startOfDay(new Date());
    const checkDate = startOfDay(date);
    if (checkDate > today) return false;
    
    // Don't show dates before employee was hired
    if (employee?.created_at) {
      const hireDate = startOfDay(parseISO(employee.created_at));
      if (checkDate < hireDate) return false;
    }

    // For dates between hire date and today (excluding holidays and weekends),
    // consider them work days
    return true;
  };

  const getWorkPercentage = (date: Date): number => {
    // Check pointage data first
    const pointageEntry = pointageData.find(p => {
      if (!p.date) return false;
      const pointageDate = parseISO(p.date);
    ...
    if (pointageEntry) {
      // jr_travaille represents the actual worked day (can be fractional like 0.5 for half day)
      const workedDay = parseFloat(pointageEntry.jr_travaille || 0);
      return Math.min(100, Math.round(workedDay * 100));
    }
    ...
    if (!schedule) return 100; // Default to full day if no specific schedule

    // Calculate percentage based on hours worked (assuming 8 hours is a full day)
    const fullDayHours = 8;
    const hoursWorked = parseFloat(schedule.hours_worked || '8');
    const percentage = Math.min(100, Math.round((hoursWorked / fullDayHours) * 100));
          ...
          <TabsContent value="calendar" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <CustomWorkCalendar
                    isWorkDay={isDateWorkDay}
                    isHoliday={isDateHoliday}
                    isWeekend={(date) => isWeekend(date) && !isDateHoliday(date)}
                    getWorkPercentage={getWorkPercentage}
          ...
          <TabsContent value="pointage" className="space-y-6">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle>Registre de pointage</CardTitle>
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-2xl font-bold">
                            {pointageData.reduce((sum, p) => sum + parseFloat(p.jr_travaille || 0), 0).toFixed(1)}
                          </p>
                        </div>
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-2xl font-bold">
                            {pointageData.filter(p => p.absent === 1 || p.absent === '1').length}
                          </p>
                        </div>
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-2xl font-bold">
                            {pointageData.reduce((sum, p) => sum + parseFloat(p.h_sup || 0), 0).toFixed(1)}h
                          </p>
                        </div>
                      ...
                      {pointageData.length > 0 ? (
                        pointageData
                          ...
                          .map((entry) => (
                            <TableRow key={entry.id}>
                              ...
                              <TableCell>{entry.horaire || '-'}</TableCell>
                              <TableCell>
                                {entry.entree ? format(parseISO(`2000-01-01T${entry.entree}`), 'HH:mm') : '-'}
                              </TableCell>
                              <TableCell>
                                {entry.sortie ? format(parseISO(`2000-01-01T${entry.sortie}`), 'HH:mm') : '-'}
                              </TableCell>
                              <TableCell className="text-center">
                                <Badge variant={parseFloat(entry.jr_travaille || 0) > 0 ? "default" : "secondary"}>
                                  {parseFloat(entry.jr_travaille || 0).toFixed(1)}
                                </Badge>
                              </TableCell>
                              <TableCell className="text-center">
                                {parseFloat(entry.retard || 0) > 0 ? (
                                  <span className="text-orange-500 font-medium">
                                    {parseFloat(entry.retard).toFixed(1)}h
                                  </span>
                                ) : (
                              ...
                              <TableCell className="text-center">
                                {parseFloat(entry.h_sup || 0) > 0 ? (
                                  <span className="text-blue-500 font-medium">
                                    {parseFloat(entry.h_sup).toFixed(1)}h
                                  </span>
                                ) : (
                                  <span className="text-muted-foreground">-</span>
                                )}
                              ...
                              <TableCell className="text-center">
                                {entry.absent === 1 || entry.absent === '1' ? (
                                  <Badge variant="destructive">
                                    <XCircle className="h-3 w-3 mr-1" />
                                    Absent
                                  </Badge>
                                ) : parseFloat(entry.jr_travaille || 0) > 0 ? (
                        ...
                        <TableRow>
                          <TableCell colSpan={8} className="text-center py-8 text-muted-foreground">
                            Aucune donnée de pointage pour ce mois
                          </TableCell>
                        </TableRow>
                      )}
                    </TableBody>
