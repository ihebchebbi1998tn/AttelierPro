    ...
    <div className="min-h-screen bg-gradient-to-br from-background to-muted/20">
      <div className="container mx-auto p-2 sm:p-4 md:p-6 space-y-3 sm:space-y-4 md:space-y-6">
        ...
        <Tabs defaultValue="overview" className="w-full">
          <TabsList className={`grid w-full ${isMobile ? 'grid-cols-1' : 'grid-cols-3 lg:grid-cols-6'} gap-1 mb-3 sm:mb-4 md:mb-6 bg-muted p-1 rounded-md overflow-x-auto`}>
          ...
          <TabsContent value="coupe" className="mt-6">
            <div className="space-y-4">
              {/* Structured Coupe Categories */}
              {order?.selected_coupe_options && order.selected_coupe_options.length > 0 && (() => {
                // Categorize options
                const categories = {
                  'Cols': ['Col Normal', 'Col Pointure', 'Croisé', 'Col Châle', 'Safari'],
                  'Poches': ['2 Poches Rabat', '3 Poches Rabat', '3 Poches Inclinée', 'Passepoil', 'Poche Plaqué'],
                  'Boutons': ['Bouton Normal', 'Bouton Métal', 'Bouton Couvert'],
                  'Pantalon': ['Pantalon Normal', 'Old Money'],
                  'Doublure': ['1/2 Doublé', 'Doublé']
                };

                const categorizedOptions: Record<string, any[]> = {};
                
                // Categorize each option
                order.selected_coupe_options.forEach((option: any) => {
                  for (const [category, items] of Object.entries(categories)) {
                    if (items.some(item => option.title?.includes(item) || item.includes(option.title))) {
                  ...
                  // If no category found, add to "Autres"
                  if (!categorizedOptions['Autres']) {
                    categorizedOptions['Autres'] = [];
                  }
                  categorizedOptions['Autres'].push(option);
                });
                        ...
                        <CardContent>
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            {options.map((option, idx) => (
                              <div key={idx} className="border rounded-lg p-3 text-center bg-muted/20 hover:bg-muted/40 transition-colors">
                                <div className="relative w-full h-40 mb-2 rounded overflow-hidden bg-background">
                                  <img 
                                    src={option.imageUrl} 
                                    alt={option.title}
                ...
                <Card>
                  ...
                  <CardContent>
                    <p className="text-muted-foreground text-center py-8">
                      Aucune option de coupe sélectionnée
                    </p>
                  </CardContent>
                </Card>
              )}
                    ...
                    {(() => {
                      ...
                      return (
                        <div>
                          ...
                          {/* Taille Générale Pantalon - Separate Table */}
                          <div className="overflow-x-auto mb-4">
                            <table className="w-full border-collapse border border-gray-200 rounded-lg">
                              ...
                              <tbody>
                                {Object.entries(tailleGeneralePantalon).map(([name, value], index) => (
                                    <tr key={name} className={`border-b ${index % 2 === 0 ? 'bg-background' : 'bg-muted/30'}`}>
                                      <td className="py-2 px-4 font-medium">{name}</td>
                                      <td className="text-right py-2 px-4">
                                        <span className="font-semibold text-primary">
